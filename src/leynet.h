...
/**
 * @file leynet.h
 * @brief Низкоуровневые сетевые обёртки (UDP/TCP) для клиента.
 *
 * Содержит классы для работы с сокетами: базовый `leynet` и
 * специализированные `leynet_udp` и `leynet_tcp`.
 *
 * Комментарии в методах дают краткое описание ожидаемого поведения
 * и возможностей (открытие/закрытие соединения, отправка/приём,
 * простая поддержка HTTP для TCP и т.д.).
 */
