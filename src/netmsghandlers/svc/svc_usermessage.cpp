...
{
	leychan::netcallbackfn fn = reinterpret_cast<leychan::netcallbackfn>(&svc_usermessage::ParseMessage);
...
bool svc_usermessage::ParseMessage(leychan* chan, svc_usermessage* thisptr, bf_read& msg)
{
	...
	if (msgtype == 40) // SendLua
	{
		int a = userMsg.ReadUBitLong(32);
		int b = userMsg.ReadByte();

		printf("SendLua Received. bits: %i | data: %s\n", bits, data);
